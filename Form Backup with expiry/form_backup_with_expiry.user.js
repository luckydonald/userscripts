    ...
    // @homepage      https://userscripts-mirror.org/scripts/show/42879
    // @downloadURL   https://raw.githubusercontent.com/luckydonald/userscripts/refs/heads/mane/Form%20Backup%20with%20expiry/form_backup_with_expiry.user.js
    // @updateURL     https://raw.githubusercontent.com/luckydonald/userscripts/refs/heads/mane/Form%20Backup%20with%20expiry/form_backup_with_expiry.user.js
    // @supportURL     https://github.com/luckydonald/userscripts/issues

    // ==/UserScript==


    // this script was originally based on https://userscripts-mirror.org/scripts/show/7671
    // and the fork of that, which is at   https://userscripts-mirror.org/scripts/show/42879

    /*

    version history

    2.5 luckydonald now took over the thing.
    ...
    */
    ...
    var expire_after_days = 1;
    var expire_after_hours = 0;
    var expire_after_minutes = 0;
    ...
    // how many times to flash. must be an even number, or the border style will not revert
    var flash_count = 6;
    ...
    var target_selector = 'input[type="text"], textarea, *[contentEditable]';
    ...
    var remove_timestamp = function (str) {
        console.log('BACKUP: remove_timestamp', { str })
    ...
    var is_content_editable = function (element) {
        return ['TEXTAREA', 'INPUT'].indexOf(element.nodeName) === -1;
    };
    ...
    var on_focus = function (event) {
        console.log('BACKUP: ELEMENT FOCUS', { element: event.target });
    ...
    var on_blur = function (event) {
        console.log('BACKUP: ELEMENT BLUR', { element: event.target });
        commit_backup(event.target);
    };

    var on_submit = function (event) {
        console.log('BACKUP: FORM SUBMIT', { element: event.target });
    ...
    var init_backup = function (element) {
        console.log('BACKUP: ENABLE ELEMENT WATCHER', { element });
    ...
    var backup_dynamic = function (evt) {
        ...
        var new_textareas = element.querySelectorAll(target_selector);
        console.log('BACKUP: CHECK FOR HTML CHANGE', { element, new_textareas });
    ...
    if (expiry_timespan > 0) {
        ...
        console.log('BACKUP: CHECK FOR EXPIRY', {curr_time, stored_bak});
        GM_setValue('last-run', curr_time);
    ...
    document.addEventListener('DOMContentLoaded', backup_dynamic);
